...
Introduce an explicit **security profile** (`FHIRENGINE_SECURITY_PROFILE = dev | production`, default
...
- **production**: the server **refuses to boot** (`process.exit(1)`) unless — authentication is
  enabled (`FHIRENGINE_AUTH_ENABLED`), audit is enabled (`FHIRENGINE_AUDIT_ENABLED`), transport security is
  present (in-process TLS **or** `FHIRENGINE_TLS_TERMINATED_AT_PROXY=true`), and OAuth signing keys are
  **non-ephemeral** when the OAuth server is on (`FHIRENGINE_OAUTH_PRIVATE_KEY/PUBLIC_KEY` set — ephemeral
  keys rotate on restart and silently invalidate live tokens).
- **consent** (`FHIRENGINE_CONSENT_ENFORCEMENT`) is **advisory even in production** — consent/DS4P/42 CFR
...
- (−) Operators must set `FHIRENGINE_SECURITY_PROFILE=production` + the required envs to run in prod; a
