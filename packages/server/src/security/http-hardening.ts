...
export function mountHttpHardening(app: Hono, opts: HardeningOptions): void {
  ...
  const origins = (env.FHIRENGINE_CORS_ORIGINS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
  if (origins.length) {
    app.use("*", cors({
      origin: origins,
  ...
  // 3) Body size limit.
  const maxBody = Number(env.FHIRENGINE_MAX_BODY_BYTES) || DEFAULT_MAX_BODY_BYTES;
  app.use("*", bodyLimit({
    maxSize: maxBody,
    onError: (c) =>
  ...
  const rlEnabled = env.FHIRENGINE_RATE_LIMIT_ENABLED === "true" || (isProd && env.FHIRENGINE_RATE_LIMIT_ENABLED !== "false");
  if (rlEnabled) {
    const rpm = Number(env.FHIRENGINE_RATE_LIMIT_RPM) || DEFAULT_RATE_LIMIT_RPM;
