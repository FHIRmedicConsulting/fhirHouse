 ...
 *
 *   FHIRENGINE_AUTH_ENABLED=true            turn the gate on (deploy gate for PHI)
 *   FHIRENGINE_AUTH_STRATEGY=stub|jwks|oidc  default 'jwks' (the ADR-0030 model); 'stub' for tests
 *   FHIRENGINE_SMART_VERSIONS=2.0.0,2.2.0    active SMART grammars (default: all supported)
 */
...
function buildStrategy(): AuthStrategy {
  switch (process.env.FHIRENGINE_AUTH_STRATEGY ?? "jwks") {
