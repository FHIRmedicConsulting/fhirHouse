 ...
 * can verify them. Loaded from `FHIRENGINE_OAUTH_PRIVATE_KEY` (PKCS8 PEM) + `FHIRENGINE_OAUTH_PUBLIC_KEY`
...
async function build(): Promise<KeyMaterial> {
  ...
  const priv = process.env.FHIRENGINE_OAUTH_PRIVATE_KEY, pub = process.env.FHIRENGINE_OAUTH_PUBLIC_KEY;
  if (priv && pub) {
