...
// --- env isolation ------------------------------------------------------------
const SECURITY_ENV = [
  "FHIRENGINE_SECURITY_PROFILE", "FHIRENGINE_AUTH_ENABLED", "FHIRENGINE_AUDIT_ENABLED", "FHIRENGINE_CONSENT_ENFORCEMENT",
  "FHIRENGINE_OAUTH_ENABLED", "FHIRENGINE_OAUTH_PRIVATE_KEY", "FHIRENGINE_OAUTH_PUBLIC_KEY", "FHIRENGINE_TLS_CERT",
  "FHIRENGINE_TLS_KEY", "FHIRENGINE_TLS_CIPHERS", "FHIRENGINE_TLS_TERMINATED_AT_PROXY", "FHIRENGINE_CORS_ORIGINS",
  "FHIRENGINE_RATE_LIMIT_ENABLED", "FHIRENGINE_RATE_LIMIT_RPM", "FHIRENGINE_MAX_BODY_BYTES",
];
...
describe("buildTlsConfig", () => {
  ...
  it("honors a FHIRENGINE_TLS_CIPHERS override", () => {
    ...
    process.env.FHIRENGINE_TLS_CERT = cert; process.env.FHIRENGINE_TLS_KEY = key;
    process.env.FHIRENGINE_TLS_CIPHERS = "ECDHE-RSA-AES256-GCM-SHA384";
...
describe("security profile", () => {
  it("defaults to dev; production only when explicitly set", () => {
    ...
    process.env.FHIRENGINE_SECURITY_PROFILE = "production";
  ...
  it("production PASSES when auth+audit on and TLS present (or proxy-terminated)", () => {
    ...
    process.env.FHIRENGINE_TLS_TERMINATED_AT_PROXY = "true";
    const r = evaluateSecurityPosture({ tlsInProcess: false });
    expect(r.ok).toBe(true);
    expect(r.errors).toEqual([]);
  });
  ...
  it("production rejects EPHEMERAL OAuth signing keys", () => {
    ...
    process.env.FHIRENGINE_AUDIT_ENABLED = "true";
    process.env.FHIRENGINE_TLS_TERMINATED_AT_PROXY = "true";
    process.env.FHIRENGINE_OAUTH_ENABLED = "true"; // enabled but no static keys → ephemeral
    const r = evaluateSecurityPosture({ tlsInProcess: false });
    expect(r.ok).toBe(false);
...
describe("mountHttpHardening", () => {
  ...
  it("dev: permissive CORS; allowlist enforced when configured", async () => {
    ...
    process.env.FHIRENGINE_CORS_ORIGINS = "https://trusted.example";
    const app = build("dev");
  ...
  it("rejects oversized bodies with 413", async () => {
    process.env.FHIRENGINE_MAX_BODY_BYTES = "16";
  ...
  it("rate-limits in the production profile", async () => {
    process.env.FHIRENGINE_RATE_LIMIT_RPM = "2";
