...
export type methodProfileRaw = {
    valueCodeableConcept: CodeableConcept;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/procedure-method (pkg: hl7.fhir.r4.core#4.0.1)
export class methodProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/procedure-method";
    ...
    static from (resource: Extension) : methodProfile {
        const profile = new methodProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : methodProfile {
        return new methodProfile(resource);
    }

    static createResource (args: methodProfileRaw) : Extension {
    ...
    static create (args: methodProfileRaw) : methodProfile {
        return methodProfile.apply(methodProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "method"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", methodProfile.canonicalUrl),
