...
export type Observation_headcircum_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_headcircumProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/headcircum (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_headcircumProfile {
    ...
    static from (resource: Observation) : observation_headcircumProfile {
        if (!resource.meta?.profile?.includes(observation_headcircumProfile.canonicalUrl)) {
            throw new Error(`observation_headcircumProfile: meta.profile must include ${observation_headcircumProfile.canonicalUrl}`)
        }
        const profile = new observation_headcircumProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_headcircumProfile {
        ensureProfile(resource, observation_headcircumProfile.canonicalUrl);
        return new observation_headcircumProfile(resource);
    }

    static createResource (args: observation_headcircumProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_headcircumProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_headcircumProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_headcircumProfileRaw) : observation_headcircumProfile {
        return observation_headcircumProfile.apply(observation_headcircumProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_headcircum_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_headcircumProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_headcircum_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_headcircum_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_headcircum_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_headcircumProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_headcircum_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-headcircum"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"9843-4","system":"http://loinc.org"}]}),
