...
export type siblingProfileRaw = {
    extension: Extension[];
}

export type siblingProfileFlat = {
...
export class siblingProfile {
    ...
    static from (resource: Extension) : siblingProfile {
        const profile = new siblingProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : siblingProfile {
        return new siblingProfile(resource);
    }

    private static resolveInput (args: siblingProfileRaw | siblingProfileFlat) : Extension[] {
        if (isRawExtensionInput<siblingProfileRaw>(args)) {
    ...
    static createResource (args: siblingProfileRaw | siblingProfileFlat) : Extension {
        const resolvedExtensions = siblingProfile.resolveInput(args ?? {});
    ...
    static create (args: siblingProfileRaw | siblingProfileFlat) : siblingProfile {
        return siblingProfile.apply(siblingProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "sibling"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "extension"),
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", siblingProfile.canonicalUrl),
            ],
            warnings: [],
        }
    }
