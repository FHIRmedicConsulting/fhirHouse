...
export type Observation_heartrate_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_heartrateProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/heartrate (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_heartrateProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/heartrate";
    ...
    static from (resource: Observation) : observation_heartrateProfile {
        if (!resource.meta?.profile?.includes(observation_heartrateProfile.canonicalUrl)) {
            throw new Error(`observation_heartrateProfile: meta.profile must include ${observation_heartrateProfile.canonicalUrl}`)
        }
        const profile = new observation_heartrateProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_heartrateProfile {
        ensureProfile(resource, observation_heartrateProfile.canonicalUrl);
        return new observation_heartrateProfile(resource);
    }

    static createResource (args: observation_heartrateProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_heartrateProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_heartrateProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_heartrateProfileRaw) : observation_heartrateProfile {
        return observation_heartrateProfile.apply(observation_heartrateProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_heartrate_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_heartrateProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_heartrate_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_heartrate_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_heartrate_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_heartrateProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_heartrate_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-heartrate"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"8867-4","system":"http://loinc.org"}]}),
