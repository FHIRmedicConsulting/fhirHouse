...
export type Observation_oxygensat_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_oxygensatProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/oxygensat (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_oxygensatProfile {
    ...
    static from (resource: Observation) : observation_oxygensatProfile {
        if (!resource.meta?.profile?.includes(observation_oxygensatProfile.canonicalUrl)) {
            throw new Error(`observation_oxygensatProfile: meta.profile must include ${observation_oxygensatProfile.canonicalUrl}`)
        }
        const profile = new observation_oxygensatProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_oxygensatProfile {
        ensureProfile(resource, observation_oxygensatProfile.canonicalUrl);
        return new observation_oxygensatProfile(resource);
    }

    static createResource (args: observation_oxygensatProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_oxygensatProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_oxygensatProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_oxygensatProfileRaw) : observation_oxygensatProfile {
        return observation_oxygensatProfile.apply(observation_oxygensatProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_oxygensat_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_oxygensatProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_oxygensat_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_oxygensat_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_oxygensat_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_oxygensatProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_oxygensat_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-oxygensat"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"2708-6","system":"http://loinc.org"}]}),
