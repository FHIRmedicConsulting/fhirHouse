...
export type systemUserLanguageProfileRaw = {
    valueCodeableConcept: CodeableConcept;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/cqf-systemUserLanguage (pkg: hl7.fhir.r4.core#4.0.1)
export class systemUserLanguageProfile {
    ...
    static from (resource: Extension) : systemUserLanguageProfile {
        const profile = new systemUserLanguageProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : systemUserLanguageProfile {
        return new systemUserLanguageProfile(resource);
    }

    static createResource (args: systemUserLanguageProfileRaw) : Extension {
    ...
    static create (args: systemUserLanguageProfileRaw) : systemUserLanguageProfile {
        return systemUserLanguageProfile.apply(systemUserLanguageProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "systemUserLanguage"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", systemUserLanguageProfile.canonicalUrl),
