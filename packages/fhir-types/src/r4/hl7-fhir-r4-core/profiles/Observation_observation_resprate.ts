...
export type Observation_resprate_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_resprateProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/resprate (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_resprateProfile {
    ...
    static from (resource: Observation) : observation_resprateProfile {
        if (!resource.meta?.profile?.includes(observation_resprateProfile.canonicalUrl)) {
            throw new Error(`observation_resprateProfile: meta.profile must include ${observation_resprateProfile.canonicalUrl}`)
        }
        const profile = new observation_resprateProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_resprateProfile {
        ensureProfile(resource, observation_resprateProfile.canonicalUrl);
        return new observation_resprateProfile(resource);
    }

    static createResource (args: observation_resprateProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_resprateProfile.VSCatSliceMatch,
        );

        const resource = buildResource<Observation>( {
            resourceType: "Observation",
            code: {"coding":[{"code":"9279-1","system":"http://loinc.org"}]},
            category: categoryWithDefaults,
            status: args.status,
            subject: args.subject,
            meta: { profile: [observation_resprateProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_resprateProfileRaw) : observation_resprateProfile {
        return observation_resprateProfile.apply(observation_resprateProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_resprate_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_resprateProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_resprate_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_resprate_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_resprate_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_resprateProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_resprate_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-resprate"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"9279-1","system":"http://loinc.org"}]}),
