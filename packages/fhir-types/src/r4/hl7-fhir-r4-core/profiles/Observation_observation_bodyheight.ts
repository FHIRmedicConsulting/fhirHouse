...
export type Observation_bodyheight_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_bodyheightProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/bodyheight (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_bodyheightProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/bodyheight";
    ...
    static from (resource: Observation) : observation_bodyheightProfile {
        if (!resource.meta?.profile?.includes(observation_bodyheightProfile.canonicalUrl)) {
            throw new Error(`observation_bodyheightProfile: meta.profile must include ${observation_bodyheightProfile.canonicalUrl}`)
        }
        const profile = new observation_bodyheightProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_bodyheightProfile {
        ensureProfile(resource, observation_bodyheightProfile.canonicalUrl);
        return new observation_bodyheightProfile(resource);
    }

    static createResource (args: observation_bodyheightProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_bodyheightProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_bodyheightProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_bodyheightProfileRaw) : observation_bodyheightProfile {
        return observation_bodyheightProfile.apply(observation_bodyheightProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_bodyheight_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_bodyheightProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_bodyheight_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_bodyheight_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_bodyheight_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_bodyheightProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_bodyheight_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-bodyheight"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"8302-2","system":"http://loinc.org"}]}),
