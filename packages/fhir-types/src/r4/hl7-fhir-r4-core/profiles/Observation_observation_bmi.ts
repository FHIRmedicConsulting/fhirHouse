...
export type Observation_bmi_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_bmiProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/bmi (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_bmiProfile {
    ...
    static from (resource: Observation) : observation_bmiProfile {
        if (!resource.meta?.profile?.includes(observation_bmiProfile.canonicalUrl)) {
            throw new Error(`observation_bmiProfile: meta.profile must include ${observation_bmiProfile.canonicalUrl}`)
        }
        const profile = new observation_bmiProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_bmiProfile {
        ensureProfile(resource, observation_bmiProfile.canonicalUrl);
        return new observation_bmiProfile(resource);
    }

    static createResource (args: observation_bmiProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_bmiProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_bmiProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_bmiProfileRaw) : observation_bmiProfile {
        return observation_bmiProfile.apply(observation_bmiProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_bmi_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_bmiProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_bmi_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_bmi_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_bmi_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_bmiProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_bmi_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-bmi"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"39156-5","system":"http://loinc.org"}]}),
