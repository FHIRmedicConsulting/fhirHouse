...
export class localProfile {
    ...
    static from (resource: Extension) : localProfile {
        const profile = new localProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : localProfile {
        return new localProfile(resource);
    }

    static createResource (args: localProfileRaw) : Extension {
    ...
    static create (args: localProfileRaw) : localProfile {
        return localProfile.apply(localProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "url", localProfile.canonicalUrl),
