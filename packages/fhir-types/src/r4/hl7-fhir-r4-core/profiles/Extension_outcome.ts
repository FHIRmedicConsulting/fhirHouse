...
export type outcomeProfileRaw = {
    valueCodeableConcept: CodeableConcept;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/condition-outcome (pkg: hl7.fhir.r4.core#4.0.1)
export class outcomeProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/condition-outcome";
    ...
    static from (resource: Extension) : outcomeProfile {
        const profile = new outcomeProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : outcomeProfile {
        return new outcomeProfile(resource);
    }

    static createResource (args: outcomeProfileRaw) : Extension {
    ...
    static create (args: outcomeProfileRaw) : outcomeProfile {
        return outcomeProfile.apply(outcomeProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "outcome"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", outcomeProfile.canonicalUrl),
