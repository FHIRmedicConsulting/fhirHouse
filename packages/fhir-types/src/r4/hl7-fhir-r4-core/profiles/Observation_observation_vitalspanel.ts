...
export type Observation_vitalspanel_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_vitalspanelProfileRaw = {
    ...
    hasMember: Reference<"MolecularSequence" | "QuestionnaireResponse" | "Observation">[];
    category?: CodeableConcept<("social-history" | "vital-signs" | "imaging" | "laboratory" | "procedure" | "survey" | "exam" | "therapy" | "activity" | string)>[];
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/vitalspanel (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_vitalspanelProfile {
    ...
    static from (resource: Observation) : observation_vitalspanelProfile {
        if (!resource.meta?.profile?.includes(observation_vitalspanelProfile.canonicalUrl)) {
            throw new Error(`observation_vitalspanelProfile: meta.profile must include ${observation_vitalspanelProfile.canonicalUrl}`)
        }
        const profile = new observation_vitalspanelProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_vitalspanelProfile {
        ensureProfile(resource, observation_vitalspanelProfile.canonicalUrl);
        return new observation_vitalspanelProfile(resource);
    }

    static createResource (args: observation_vitalspanelProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_vitalspanelProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_vitalspanelProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_vitalspanelProfileRaw) : observation_vitalspanelProfile {
        return observation_vitalspanelProfile.apply(observation_vitalspanelProfile.createResource(args));
    }
    ...
    getHasMember () : Reference<"MolecularSequence" | "QuestionnaireResponse" | "Observation">[] | undefined {
        return this.resource.hasMember as Reference<"MolecularSequence" | "QuestionnaireResponse" | "Observation">[] | undefined;
    }

    setHasMember (value: Reference<"MolecularSequence" | "QuestionnaireResponse" | "Observation">[]) : this {
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_vitalspanel_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_vitalspanelProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_vitalspanel_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_vitalspanel_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_vitalspanel_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_vitalspanelProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_vitalspanel_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-vitalspanel"
