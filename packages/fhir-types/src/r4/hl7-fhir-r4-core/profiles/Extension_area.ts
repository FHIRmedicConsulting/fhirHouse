...
export type areaProfileRaw = {
    valueString: string;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/contactpoint-area (pkg: hl7.fhir.r4.core#4.0.1)
export class areaProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/contactpoint-area";
    ...
    static from (resource: Extension) : areaProfile {
        const profile = new areaProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : areaProfile {
        return new areaProfile(resource);
    }

    static createResource (args: areaProfileRaw) : Extension {
    ...
    static create (args: areaProfileRaw) : areaProfile {
        return areaProfile.apply(areaProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "url", areaProfile.canonicalUrl),
