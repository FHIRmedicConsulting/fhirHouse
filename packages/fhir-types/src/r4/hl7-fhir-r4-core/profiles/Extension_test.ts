...
export type testProfileRaw = {
    valueReference: Reference;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/openEHR-test (pkg: hl7.fhir.r4.core#4.0.1)
export class testProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/openEHR-test";
    ...
    static from (resource: Extension) : testProfile {
        const profile = new testProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : testProfile {
        return new testProfile(resource);
    }

    static createResource (args: testProfileRaw) : Extension {
    ...
    static create (args: testProfileRaw) : testProfile {
        return testProfile.apply(testProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "url", testProfile.canonicalUrl),
