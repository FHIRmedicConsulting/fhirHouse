...
export type adoptionInfoProfileRaw = {
    valueCodeableConcept: CodeableConcept;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/patient-adoptionInfo (pkg: hl7.fhir.r4.core#4.0.1)
export class adoptionInfoProfile {
    ...
    static from (resource: Extension) : adoptionInfoProfile {
        const profile = new adoptionInfoProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : adoptionInfoProfile {
        return new adoptionInfoProfile(resource);
    }

    static createResource (args: adoptionInfoProfileRaw) : Extension {
    ...
    static create (args: adoptionInfoProfileRaw) : adoptionInfoProfile {
        return adoptionInfoProfile.apply(adoptionInfoProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "adoptionInfo"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", adoptionInfoProfile.canonicalUrl),
