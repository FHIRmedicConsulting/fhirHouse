...
export class riskProfile {
    ...
    static from (resource: Extension) : riskProfile {
        const profile = new riskProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : riskProfile {
        return new riskProfile(resource);
    }

    static createResource (args: riskProfileRaw) : Extension {
    ...
    static create (args: riskProfileRaw) : riskProfile {
        return riskProfile.apply(riskProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "risk"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", riskProfile.canonicalUrl),
