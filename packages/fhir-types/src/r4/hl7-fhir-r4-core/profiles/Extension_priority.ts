...
export type priorityProfileRaw = {
    valueCodeableConcept: CodeableConcept;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/flag-priority (pkg: hl7.fhir.r4.core#4.0.1)
export class priorityProfile {
    ...
    static from (resource: Extension) : priorityProfile {
        const profile = new priorityProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : priorityProfile {
        return new priorityProfile(resource);
    }

    static createResource (args: priorityProfileRaw) : Extension {
    ...
    static create (args: priorityProfileRaw) : priorityProfile {
        return priorityProfile.apply(priorityProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "priority"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", priorityProfile.canonicalUrl),
