...
export type labelProfileRaw = {
    valueString: string;
}

// CanonicalURL: http://hl7.org/fhir/StructureDefinition/codesystem-label (pkg: hl7.fhir.r4.core#4.0.1)
export class labelProfile {
    ...
    static from (resource: Extension) : labelProfile {
        const profile = new labelProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Extension) : labelProfile {
        return new labelProfile(resource);
    }

    static createResource (args: labelProfileRaw) : Extension {
    ...
    static create (args: labelProfileRaw) : labelProfile {
        return labelProfile.apply(labelProfile.createResource(args));
    }
    ...
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "label"
        const res = this.resource
        return {
            errors: [
                ...validateRequired(res, profileName, "url"),
                ...validateFixedValue(res, profileName, "url", labelProfile.canonicalUrl),
