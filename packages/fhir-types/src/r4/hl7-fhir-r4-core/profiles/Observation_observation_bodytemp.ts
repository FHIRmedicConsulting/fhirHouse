...
export type Observation_bodytemp_Category_VSCatSliceFlat = Omit<CodeableConcept, "coding">;
...
export type observation_bodytempProfileRaw = {
...
// CanonicalURL: http://hl7.org/fhir/StructureDefinition/bodytemp (pkg: hl7.fhir.r4.core#4.0.1)
export class observation_bodytempProfile {
    static readonly canonicalUrl = "http://hl7.org/fhir/StructureDefinition/bodytemp";
    ...
    static from (resource: Observation) : observation_bodytempProfile {
        if (!resource.meta?.profile?.includes(observation_bodytempProfile.canonicalUrl)) {
            throw new Error(`observation_bodytempProfile: meta.profile must include ${observation_bodytempProfile.canonicalUrl}`)
        }
        const profile = new observation_bodytempProfile(resource);
        const { errors } = profile.validate();
        if (errors.length > 0) throw new Error(errors.join("; "))
        return profile;
    }

    static apply (resource: Observation) : observation_bodytempProfile {
        ensureProfile(resource, observation_bodytempProfile.canonicalUrl);
        return new observation_bodytempProfile(resource);
    }

    static createResource (args: observation_bodytempProfileRaw) : Observation {
        const categoryWithDefaults = ensureSliceDefaults(
            [...(args.category ?? [])],
            observation_bodytempProfile.VSCatSliceMatch,
        );
        ...
        const resource = buildResource<Observation>( {
            ...
            meta: { profile: [observation_bodytempProfile.canonicalUrl] },
        })
        return resource;
    }

    static create (args: observation_bodytempProfileRaw) : observation_bodytempProfile {
        return observation_bodytempProfile.apply(observation_bodytempProfile.createResource(args));
    }
    ...
    // Extensions
    // Slices
    public setVSCat (input?: Observation_bodytemp_Category_VSCatSliceFlat | CodeableConcept): this {
        const match = observation_bodytempProfile.VSCatSliceMatch
    ...
    public getVSCat(mode: 'flat'): Observation_bodytemp_Category_VSCatSliceFlat | undefined;
    public getVSCat(mode: 'raw'): CodeableConcept | undefined;
    public getVSCat(): Observation_bodytemp_Category_VSCatSliceFlat | undefined;
    public getVSCat (mode: 'flat' | 'raw' = 'flat'): Observation_bodytemp_Category_VSCatSliceFlat | CodeableConcept | undefined {
        const match = observation_bodytempProfile.VSCatSliceMatch
        const item = getArraySlice(this.resource.category, match)
        if (!item) return undefined
        if (mode === 'raw') return item
        return stripMatchKeys<Observation_bodytemp_Category_VSCatSliceFlat>(item, ["coding"])
    }

    // Validation
    validate(): { errors: string[]; warnings: string[] } {
        const profileName = "observation-bodytemp"
        ...
        return {
            errors: [
                ...
                ...validateFixedValue(res, profileName, "code", {"coding":[{"code":"8310-5","system":"http://loinc.org"}]}),
