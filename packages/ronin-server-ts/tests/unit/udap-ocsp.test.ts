...
afterAll(() => { delete process.env.RONIN_UDAP_OCSP_URLS; delete process.env.RONIN_UDAP_OCSP_HARD_FAIL; delete process.env.RONIN_UDAP_OCSP_CHECK; });

const checker = (fetch: () => Promise<Uint8Array>, env: Record<string, string> = {}) =>
  new OcspRevocationChecker(async () => fetch().then((b) => b), { RONIN_UDAP_OCSP_URLS: OCSP_URL, ...env } as NodeJS.ProcessEnv);
...
describe.skipIf(!ok)("live OCSP revocation", () => {
  ...
  it("hard-fail rejects when the responder is unreachable", async () => {
    const c = checker(async () => { throw new Error("down"); }, { RONIN_UDAP_OCSP_HARD_FAIL: "true" });
