...
describe.skipIf(!SIDECAR)("SMART authorization server (end-to-end)", () => {
  ...
  beforeAll(async () => {
    ...
    process.env.RONIN_OAUTH_DEFAULT_PATIENT = P1;
    app = createDeltaApp({ warehouse: wh, baseUrl: URLBASE });
  });
  afterAll(() => { for (const k of ["RONIN_OAUTH_ENABLED", "RONIN_AUTH_ENABLED", "RONIN_AUTH_STRATEGY", "RONIN_OAUTH_DEFAULT_PATIENT"]) delete process.env[k]; });
