...
describe.skipIf(!SIDECAR)("SMART Backend Services (client_credentials + private_key_jwt)", () => {
  ...
  beforeAll(async () => {
    ...
    delete process.env.RONIN_AUTH_ENABLED; delete process.env.RONIN_OAUTH_ENABLED;
    ...
    resetKeys();
    process.env.RONIN_OAUTH_CLIENTS = JSON.stringify([{ clientId: CID, type: "confidential", jwks: { keys: [{ ...jwk, kid, alg: "RS256", use: "sig" }] } }]);
    process.env.RONIN_OAUTH_ENABLED = "true";
    process.env.RONIN_AUTH_ENABLED = "true";
    process.env.RONIN_AUTH_STRATEGY = "local";
    app = createDeltaApp({ warehouse: wh, baseUrl: URLBASE });
  });
  afterAll(() => { for (const k of ["RONIN_OAUTH_CLIENTS", "RONIN_OAUTH_ENABLED", "RONIN_AUTH_ENABLED", "RONIN_AUTH_STRATEGY"]) delete process.env[k]; });
