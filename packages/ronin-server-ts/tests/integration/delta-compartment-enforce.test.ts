...
describe.skipIf(!SIDECAR)("patient-compartment enforcement (no cross-patient leakage)", () => {
  ...
  beforeAll(async () => {
    ...
    delete process.env.RONIN_AUTH_ENABLED;                 // seed with auth OFF
    ...
    process.env.RONIN_AUTH_ENABLED = "true";               // now build the enforcing app
    process.env.RONIN_AUTH_STRATEGY = "stub";
    appAuth = createDeltaApp({ warehouse: wh, baseUrl: "http://test" });
  });
  afterAll(() => { delete process.env.RONIN_AUTH_ENABLED; delete process.env.RONIN_AUTH_STRATEGY; });
