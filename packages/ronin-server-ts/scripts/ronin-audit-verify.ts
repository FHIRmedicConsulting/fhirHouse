/**
 * ronin-audit-verify — verify the tamper-evidence of the AuditEvent hash chain (ADR-0035).
 * Reports whether the audit log has been edited, had records deleted, or forked. Read-only.
 *
 * Usage:
 *   RONIN_DELTA_SIDECAR_URL=... RONIN_DELTA_BASE=... npx tsx scripts/ronin-audit-verify.ts
...
async function main(): Promise<void> {
  const wh = new DeltaWarehouse({
    sidecarUrl: process.env.RONIN_DELTA_SIDECAR_URL ?? "http://127.0.0.1:8077",
    base: process.env.RONIN_DELTA_BASE ?? "./delta",
  });
  if (!(await wh.health())) throw new Error("delta sidecar not reachable (set RONIN_DELTA_SIDECAR_URL)");
