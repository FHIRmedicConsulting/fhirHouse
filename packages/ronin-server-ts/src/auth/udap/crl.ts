 ...
 *
 * Opt-in: `RONIN_UDAP_CRL_CHECK=true`. Extra/override CRL URLs: `RONIN_UDAP_CRL_URLS` (comma-sep).
 * Soft-fail by default (a CRL we can't fetch/verify → "unknown", not rejected) so a down CRL server
 * doesn't block all B2B; `RONIN_UDAP_CRL_HARD_FAIL=true` flips it to fail-closed.
 *
...
export function crlUrlsFor(cert: X509Certificate, env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  for (const u of (env.RONIN_UDAP_CRL_URLS ?? "").split(",").map((s) => s.trim()).filter(Boolean)) urls.add(u);
  return [...urls];
}
...
export class CrlRevocationChecker {
  ...
  async isRevoked(cert: X509Certificate, issuers: X509Certificate[]): Promise<CrlResult> {
    const hardFail = this.env.RONIN_UDAP_CRL_HARD_FAIL === "true";
...
export const crlCheckEnabled = (env: NodeJS.ProcessEnv = process.env): boolean => env.RONIN_UDAP_CRL_CHECK === "true";
