 ...
 * the in-process public key (no network / JWKS fetch). Use `RONIN_AUTH_STRATEGY=local` to close
 * the loop: our authorization server issues the token, our gate verifies + enforces it.
 * (For an external IdP, use the `jwks` strategy with RONIN_JWKS_URI instead.)
 */
