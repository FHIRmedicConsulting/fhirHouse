 ...
 *
 *   RONIN_AUDIT_ANCHOR_WEBHOOK        POST each anchor here (the external immutable sink)
 *   RONIN_AUDIT_ANCHOR_KEY            PEM (PKCS8) private key → anchors are signed (JWS); optional
 *   RONIN_AUDIT_ANCHOR_INTERVAL_MIN   periodic anchoring interval (opt-in; off if unset)
 *
...
/** Load the anchor signing key from RONIN_AUDIT_ANCHOR_KEY (PEM PKCS8), or null. */
async function anchorKey(env: NodeJS.ProcessEnv): Promise<Awaited<ReturnType<typeof importPKCS8>> | null> {
  const pem = env.RONIN_AUDIT_ANCHOR_KEY;
  if (!pem) return null;
...
export async function publishAnchor(anchor: AuditAnchor, env: NodeJS.ProcessEnv = process.env): Promise<void> {
  const url = env.RONIN_AUDIT_ANCHOR_WEBHOOK;
  if (!url) return;
...
/**
 * Opt-in periodic anchoring: compute + publish an anchor every RONIN_AUDIT_ANCHOR_INTERVAL_MIN.
...
export function startAuditAnchorScheduler(wh: AuditBackend, log?: AnchorLogger, env: NodeJS.ProcessEnv = process.env): (() => void) | null {
  const minutes = Number(env.RONIN_AUDIT_ANCHOR_INTERVAL_MIN);
