 ...
 * *fails closed*: when `RONIN_SECURITY_PROFILE=production`, the server REFUSES TO BOOT unless
 * the required controls are on.
 *
 *   RONIN_SECURITY_PROFILE = dev (default) | production
 *   RONIN_TLS_TERMINATED_AT_PROXY = true    operator attests TLS is terminated upstream
 *                                           (satisfies the transmission-security requirement
 *                                            when RONIN_TLS_CERT/KEY are not set on this process)
 *
...
export function securityProfile(env: NodeJS.ProcessEnv = process.env): SecurityProfile {
  return env.RONIN_SECURITY_PROFILE === "production" ? "production" : "dev";
}
...
export function evaluateSecurityPosture(input: PostureInput): PostureResult {
  ...
  const tlsSatisfied = input.tlsInProcess || env.RONIN_TLS_TERMINATED_AT_PROXY === "true";
  const oauthOn = oauthEnabled();
  const ephemeralOauthKeys = oauthOn && !(env.RONIN_OAUTH_PRIVATE_KEY && env.RONIN_OAUTH_PUBLIC_KEY);
  ...
  const checks: Array<{ unmet: boolean; msg: string; hardEvenInDev?: boolean }> = [
    { unmet: !authEnabled(), msg: "authentication is disabled (set RONIN_AUTH_ENABLED=true) — §164.312(a)(d)" },
    { unmet: !auditEnabled(), msg: "audit logging is disabled (set RONIN_AUDIT_ENABLED=true) — §164.312(b)" },
    { unmet: !tlsSatisfied, msg: "no transport security: set RONIN_TLS_CERT/KEY, or RONIN_TLS_TERMINATED_AT_PROXY=true if a proxy terminates TLS — §164.312(e)" },
    { unmet: ephemeralOauthKeys, msg: "OAuth server uses EPHEMERAL signing keys (rotate on restart, invalidating live tokens): set RONIN_OAUTH_PRIVATE_KEY/PUBLIC_KEY" },
    // Consent is deployment-dependent (not every server segments on consent) → advisory even in prod.
    { unmet: !consentEnabled(), msg: "consent enforcement is off (RONIN_CONSENT_ENFORCEMENT) — required if serving data subject to consent/DS4P/42 CFR Part 2", hardEvenInDev: false },
  ];

  const advisoryOnly = new Set([
    "consent enforcement is off (RONIN_CONSENT_ENFORCEMENT) — required if serving data subject to consent/DS4P/42 CFR Part 2",
  ]);
